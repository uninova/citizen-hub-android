...
public interface StepsSnapshotMeasurementDao {
    ...
    LiveData<List<StepsSnapshotMeasurementRecord>> get(LocalDate from, LocalDate to);
    ...
    @TypeConverters(EpochTypeConverter.class)
    LiveData<Integer> getMaximum(LocalDate from, LocalDate to);
    ...
    @TypeConverters(EpochTypeConverter.class)
    LiveData<WalkingInformation> getLatest(LocalDate from, LocalDate to);

    @Insert
    void insert(StepsSnapshotMeasurementRecord record);

    @Query("INSERT INTO steps_snapshot_measurement (sample_id, type, value) VALUES (:sampleId, :type, :value)")
    void insert(Long sampleId, Integer type, Integer value);
}
