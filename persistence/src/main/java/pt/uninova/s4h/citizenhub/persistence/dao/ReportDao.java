...
public interface ReportDao {
    ...
    @TypeConverters(EpochTypeConverter.class)
    List<ReportUtil> getWorkTimeSimpleRecords(LocalDate from, LocalDate to);
    ...
    @Query("SELECT dailyCalories.calories, dailyDistance.distance, "
            ...
            + " AND NOT EXISTS "
            + " (SELECT tag.label FROM tag INNER JOIN sample ON tag.sample_id = sample.id WHERE tag.label = 1)) dailyCalories, "

            + " (SELECT MAX(value) AS distance FROM distance_snapshot_measurement "
            + " INNER JOIN sample ON distance_snapshot_measurement.sample_id = sample.id "
            + " WHERE sample.timestamp >= :from AND sample.timestamp < :to "
            + " AND NOT EXISTS "
            + " (SELECT tag.label FROM tag INNER JOIN sample ON tag.sample_id = sample.id WHERE tag.label = 1)) dailyDistance, "

            + " (SELECT MAX(value) AS maxHeartRate, MIN(value) AS minHeartRate, AVG(value) AS avgHeartRate FROM heart_rate_measurement "
            + " INNER JOIN sample ON heart_rate_measurement.sample_id = sample.id "
            + " WHERE sample.timestamp >= :from AND sample.timestamp < :to "
            + " AND NOT EXISTS "
            + " (SELECT tag.label FROM tag INNER JOIN sample ON tag.sample_id = sample.id WHERE tag.label = 1)) heartRate, "
            /*+ " (SELECT classification AS classification, SUM(duration) AS dailyPostureDuration "
            + " FROM posture_measurement INNER JOIN sample ON posture_measurement.sample_id = sample.id "
            + " WHERE sample.timestamp >= :from AND sample.timestamp < :to GROUP BY classification) posture, "*/
            + " (SELECT classification AS classification, SUM(duration) AS correctPostureDuration FROM posture_measurement "
            ...
            + " AND NOT EXISTS "
            + " (SELECT tag.label FROM tag INNER JOIN sample ON tag.sample_id = sample.id WHERE tag.label = 1)) correctPosture, "

            + " (SELECT classification AS classification, SUM(duration) AS wrongPostureDuration FROM posture_measurement "
            + " INNER JOIN sample ON posture_measurement.sample_id = sample.id "
            + " WHERE sample.timestamp >= :from AND sample.timestamp < :to AND classification = 2 "
            + " AND NOT EXISTS "
            + "(SELECT tag.label FROM tag INNER JOIN sample ON tag.sample_id = sample.id WHERE tag.label = 1)) wrongPosture, "

            + "(SELECT MAX(value) AS steps FROM steps_snapshot_measurement "
            + " INNER JOIN sample ON steps_snapshot_measurement.sample_id = sample.id "
            + " WHERE sample.timestamp >= :from AND sample.timestamp < :to "
            + " AND NOT EXISTS "
            + "(SELECT tag.label FROM tag INNER JOIN sample ON tag.sample_id = sample.id WHERE tag.label = 1)) dailySteps")
    @TypeConverters(EpochTypeConverter.class)
    List<ReportUtil> getNotWorkTimeSimpleRecords(LocalDate from, LocalDate to);

    @Query("SELECT diastolic, systolic, mean_arterial_pressure AS meanArterialPressure, sample.timestamp AS timestamp FROM blood_pressure_measurement INNER JOIN sample ON blood_pressure_measurement.sample_id = sample.id WHERE sample.timestamp >= :from AND sample.timestamp < :to ORDER BY timestamp")
    @TypeConverters(EpochTypeConverter.class)
    ...
    @Query("SELECT diastolic, systolic, mean_arterial_pressure AS meanArterialPressure, sample.timestamp AS timestamp, "
            ...
            + " LEFT JOIN pulse_rate_measurement ON sample.id = pulse_rate_measurement.sample_id "
            + " INNER JOIN tag ON sample.id = tag.sample_id "
    ...
    @Query("SELECT diastolic, systolic, mean_arterial_pressure AS meanArterialPressure, sample.timestamp AS timestamp, "
            ...
            + " WHERE sample.timestamp >= :from AND sample.timestamp < :to "
            + " AND NOT EXISTS "
            + "(SELECT tag.label FROM tag INNER JOIN sample ON tag.sample_id = sample.id WHERE tag.label = 1) ORDER BY timestamp")
    @TypeConverters(EpochTypeConverter.class)
    ...
    @Query("SELECT duration AS lumbarExtensionDuration, score AS lumbarExtensionScore, repetitions AS lumbarExtensionRepetitions, weight AS lumbarExtensionWeight, sample.timestamp AS timestamp, calories_measurement.value AS calories FROM lumbar_extension_training_measurement INNER JOIN sample ON lumbar_extension_training_measurement.sample_id = sample.id LEFT JOIN calories_measurement ON sample.id = calories_measurement.sample_id WHERE sample.timestamp >= :from AND sample.timestamp < :to ORDER BY timestamp")
    @TypeConverters(EpochTypeConverter.class)
