...
import static datastorage.DeviceContract.*;
...
/**
    ...
    private static final String AND = " AND ";
        ...
        private Uri insertDevice(Uri uri, ContentValues values) {
            ...
            Integer isconnected = values.getAsInteger(DeviceEntry.COLUMN_IS_CONNECTED);

            if (isconnected != null && isconnected < 0) {
    ...
    private int updateDevice(Uri uri, ContentValues values, String selection, String[] selectionArgs) {
        ...
        if (values.containsKey(DeviceEntry.COLUMN_IS_CONNECTED)) {
            Integer isconnected = values.getAsInteger(DeviceEntry.COLUMN_IS_CONNECTED);

            if (isconnected == null || isconnected<=-1 ) {
                throw new IllegalArgumentException("Device requires a connection State");
            }
        }
